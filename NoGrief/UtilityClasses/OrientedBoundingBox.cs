namespace NoGriefPlugin.UtilityClasses
{
    ...
    public struct OrientedBoundingBoxD : IEquatable<OrientedBoundingBoxD>
    {
        #region Constants
        public const int CornerCount = 8;
        // Epsilon value used in ray tests, where a ray might hit the box almost edge-on.
        ...
        /// Create an oriented box with the given center, half-extents, and orientation.
        /// </summary>
        public OrientedBoundingBoxD( Vector3D center, Vector3D halfExtents, Quaternion orientation )
        {
            Center = center;
            HalfExtent = halfExtents;
            Orientation = orientation;
        ...
        /// Create an oriented box from an axis-aligned box.
        /// </summary>
        public static OrientedBoundingBoxD CreateFromBoundingBox( BoundingBoxD box )
        {
            Vector3D mid = (box.Min + box.Max) * 0.5f;
            Vector3D halfExtent = (box.Max - box.Min) * 0.5f;
            return new OrientedBoundingBoxD( mid, halfExtent, Quaternion.Identity );
        }
        /// <summary>
        /// Transform the given bounding box by a rotation around the origin followed by a translation
        /// </summary>
        ...
        /// <param name="translation"></param>
        /// <returns>A new bounding box, transformed relative to this one</returns>
        public OrientedBoundingBoxD Transform( Quaternion rotation, Vector3 translation )
        {
            return new OrientedBoundingBoxD( Vector3.Transform( Center, rotation ) + translation,
            HalfExtent,
            Orientation * rotation );
        }
        /// <summary>
        ...
        /// </summary>
        /// <returns>A new bounding box, transformed relative to this one</returns>
        public OrientedBoundingBoxD Transform( float scale, Quaternion rotation, Vector3 translation )
        {
            return new OrientedBoundingBoxD( Vector3D.Transform( Center * scale, rotation ) + translation,
            HalfExtent * scale,
            Orientation * rotation );
        }
        #endregion
        #region IEquatable implementation
        public bool Equals( OrientedBoundingBoxD other )
        {
            return (Center == other.Center && HalfExtent == other.HalfExtent && Orientation == other.Orientation);
        }
        public override bool Equals( Object obj )
        {
            ...
            {
                OrientedBoundingBoxD other = (OrientedBoundingBoxD)obj;
                return (Center == other.Center && HalfExtent == other.HalfExtent && Orientation == other.Orientation);
            }
            else
            {
        ...
        public static bool operator ==( OrientedBoundingBoxD a, OrientedBoundingBoxD b )
        {
            return Equals( a, b );
        }
        public static bool operator !=( OrientedBoundingBoxD a, OrientedBoundingBoxD b )
        ...
        /// Determine if box A contains, intersects, or is disjoint from box B.
        /// </summary>
        public static ContainmentType Contains( ref BoundingBoxD boxA, ref OrientedBoundingBoxD oboxB )
        {
            Vector3D boxA_halfExtent = (boxA.Max - boxA.Min) * 0.5f;
        ...
        /// Returns true if this box intersects the given other box.
        /// </summary>
        public bool Intersects( ref OrientedBoundingBoxD other )
        {
            return Contains( ref other ) != ContainmentType.Disjoint;
        }
        /// <summary>
        ...
        /// the given other box.
        /// </summary>
        public ContainmentType Contains( ref OrientedBoundingBoxD other )
        {
            // Build the 3x3 rotation matrix that defines the orientation of 'other' relative to this box
            Quaternion invOrient;
            Quaternion.Conjugate( ref Orientation, out invOrient );
        ...
        /// the given oriented box.
        /// </summary>
        public static ContainmentType Contains( BoundingFrustumD frustum, ref OrientedBoundingBoxD obox )
        {
            return frustum.Contains( obox.ConvertToFrustum( ) );
        }
        #endregion
        ...
        public ContainmentType Contains( ref BoundingSphereD sphere )
        {
            // Transform the sphere into local box space
            Quaternion iq = Quaternion.Conjugate( Orientation );
            Vector3D localCenter = Vector3D.Transform( sphere.Center - Center, iq );
            // (dx,dy,dz) = signed distance of center of sphere from edge of box
        ...
        /// Test whether a BoundingSphere contains, intersects, or is disjoint from a BoundingOrientedBox
        /// </summary>
        public static ContainmentType Contains( ref BoundingSphereD sphere, ref OrientedBoundingBoxD box )
        {
            // Transform the sphere into local box space
            Quaternion iq = Quaternion.Conjugate( box.Orientation );
            Vector3D localCenter = Vector3D.Transform( sphere.Center - box.Center, iq );
            localCenter.X = Math.Abs( localCenter.X );
            localCenter.Y = Math.Abs( localCenter.Y );
            localCenter.Z = Math.Abs( localCenter.Z );
            // Check for box completely inside sphere
            ...
            if ( (localCenter + box.HalfExtent).LengthSquared( ) <= rSquared )
                return ContainmentType.Contains;
            // (dx,dy,dz) = signed distance of center of sphere from edge of box
            Vector3D d = localCenter - box.HalfExtent;
            // Compute how far away the sphere is in each dimension
            d.X = Math.Max( d.X, 0.0f );
            d.Y = Math.Max( d.Y, 0.0f );
            d.Z = Math.Max( d.Z, 0.0f );
        ...
        public bool Contains( ref Vector3D point )
        {
            ...
            // our extents.
            Quaternion qinv = Quaternion.Conjugate( Orientation );
            Vector3D plocal = Vector3.Transform( point - Center, qinv );
            return Math.Abs( plocal.X ) <= HalfExtent.X &&
            Math.Abs( plocal.Y ) <= HalfExtent.Y &&
            Math.Abs( plocal.Z ) <= HalfExtent.Z;
        }
        ...
        public double? Intersects( ref RayD ray )
        {
            ...
            // Y-case
            axisDotOrigin = Vector3D.Dot( R.Up, TOrigin );
            axisDotDir = Vector3D.Dot( R.Up, ray.Direction );
            if ( axisDotDir >= -RAY_EPSILON && axisDotDir <= RAY_EPSILON )
            {
                if ( (-axisDotOrigin - HalfExtent.Y) > 0.0 || (-axisDotOrigin + HalfExtent.Y) > 0.0f )
                    return null;
            ...
            // Z-case
            axisDotOrigin = Vector3D.Dot( R.Forward, TOrigin );
            axisDotDir = Vector3D.Dot( R.Forward, ray.Direction );
            if ( axisDotDir >= -RAY_EPSILON && axisDotDir <= RAY_EPSILON )
            {
                if ( (-axisDotOrigin - HalfExtent.Z) > 0.0 || (-axisDotOrigin + HalfExtent.Z) > 0.0f )
                    return null;
        ...
        {
            Vector3D[ ] corners = new Vector3D[CornerCount];
            GetCorners( corners, 0 );
            return corners;
        }
        /// <summary>
        ...
        public BoundingFrustumD ConvertToFrustum( )
        {
            Quaternion invOrientation;
            Quaternion.Conjugate( ref Orientation, out invOrientation );
            double sx = 1.0f / HalfExtent.X;
            double sy = 1.0f / HalfExtent.Y;
            double sz = .5f / HalfExtent.Z;
            MatrixD temp;
            ...
            temp.M23 *= sz;
            temp.M33 *= sz;
            temp.Translation = Vector3D.UnitZ * 0.5f + Vector3D.TransformNormal( -Center, temp );
